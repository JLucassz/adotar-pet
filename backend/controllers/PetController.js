...
module.exports = class PetController {
  ...
  static async getAll(req, res) {
    // resgatar todos os pets em ordem de criaçao
  ...
  static async schedule(req, res) {
    ...
    // Checando se o usuario registrou o pet
    const token = getToken(req);
    const user = await getUserByToken(token);

    if (pet.user._id.equals(user._id)) {
      res.status(422).json({
        message: "Você não pode agendar uma visitar com seu próprio pet!",
      });
      return;
    }

    // Checar se o usuario ja marcou uma visita
    if (pet.adopter) {
    ...
    // Adicionar usuario como adodante do pet
    pet.adopter = {
      _id: user._id,
      name: user.name,
      image: user.image,
